/**
 * LeadEmptyState Component
 * Displays helpful messaging when lead list is empty or when search/filter returns no records.
 */
  ...
  return (
    <div className="rounded-2xl bg-white border border-slate-200/80 p-8 sm:p-12 text-center shadow-2xs">
      ...
      <h3 className="text-base font-bold text-slate-900 mb-1">
        {isFiltered ? 'No leads match your search or filter' : 'No leads found'}
      </h3>
      <p className="text-xs sm:text-sm text-slate-500 max-w-sm mx-auto mb-5">
        {isFiltered
          ? 'Try adjusting your search query or reset the status filter to see available leads.'
          : 'Get started by tracking your first sales prospect or lead.'}
      </p>
