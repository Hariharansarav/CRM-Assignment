...
/**
 * DeleteOpportunityDialog Component
 * Confirmation dialog for safely deleting an opportunity record.
 */
export const DeleteOpportunityDialog = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(
        err.message || 'Unable to delete opportunity. Please try again.'
      );
      setDeleting(false);
    }
  };
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="delete-opp-title"
    >
      <div className="relative w-full max-w-md rounded-2xl bg-white border border-slate-200 shadow-xl overflow-hidden p-5 sm:p-6 text-left">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Are you sure you want to delete <strong className="text-slate-800">{opportunity.name}</strong>? This deal will be removed permanently from your sales pipeline.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-2.5">
          ...
          >
            ...
            <span>{deleting ? 'Deleting...' : 'Delete Opportunity'}</span>
          </button>
        </div>
      </div>
    </div>
