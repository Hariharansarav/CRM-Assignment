 ...
 * Search input (debounced by 300ms) and status dropdown (Prospecting, Proposal, Negotiation, Won, Lost).
 */
export const OpportunityFilters = ({
  ...
  // Sync state during render when prop changes externally
  ...
  return (
    <div className="rounded-2xl bg-white p-3.5 sm:p-4 border border-slate-200/80 shadow-2xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 min-w-0">
      ...
      {/* Right Controls: Status Filter & Optional Reset Button */}
      <div className="flex items-center gap-2.5 shrink-0 self-start sm:self-auto min-w-0">
        <div className="relative">
          <select
            value={status}
            onChange={(e) => onStatusChange(e.target.value)}
            aria-label="Filter opportunities by status stage"
